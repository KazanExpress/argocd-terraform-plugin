...
`argocd-terraform-plugin generate ./ | kubectl apply -f -`
...
`argocd-terraform-plugin generate - < example.yaml | kubectl apply -f -`
...
1. Select your plugin via the UI by selecting `New App` and then changing `Directory` at the bottom of the form to be `argocd-terraform-plugin`.

2. Apply a Argo CD Application yaml that has `argocd-terraform-plugin` as the plugin.
```yaml
...
spec:
  ...
  source:
    path: .
    plugin:
      name: argocd-terraform-plugin # Don't include this if installing via sidecar
...
`argocd app create you-app-name --config-management-plugin argocd-terraform-plugin`

#### With Helm
If you want to use Helm along with argocd-terraform-plugin, use the instructions matching your [plugin installation method](../installation).
...
configManagementPlugins: |
  - name: argocd-terraform-plugin-helm
    ...
    generate:
      command: ["sh", "-c"]
      args: ["helm template $ARGOCD_APP_NAME . --include-crds | argocd-terraform-plugin generate -"]
```
...
If you want to use Helm along with argocd-terraform-plugin and use additional helm args,
...
configManagementPlugins: |
  - name: argocd-terraform-plugin-helm
    ...
    generate:
      command: ["sh", "-c"]
      args: ["helm template $ARGOCD_APP_NAME ${helm_args} . --include-crds | argocd-terraform-plugin generate -"]
```
For sidecar configured plugins, add this to `cmp-plugin` ConfigMap, and then [add a sidecar to run it](../installation#initcontainer-and-configuration-via-sidecar):
```yaml
  avp-helm.yaml: |
...
configManagementPlugins: |
  - name: argocd-terraform-plugin-helm
    generate:
      command: ["bash", "-c"]
      args: ['helm template "$ARGOCD_APP_NAME" -f <(echo "$HELM_VALUES") . | argocd-terraform-plugin generate -']
```
For sidecar configured plugins, add this to `cmp-plugin` ConfigMap, and then [add a sidecar to run it](../installation#initcontainer-and-configuration-via-sidecar):
```yaml
  avp-helm.yaml: |
...
#### With Kustomize
If you want to use Kustomize along with argocd-terraform-plugin, use the instructions matching your [plugin installation method](../installation).
...
#### With Jsonnet
If you want to use Jsonnet along with argocd-terraform-plugin, use the instructions matching your [plugin installation method](../installation).
...
```yaml
configManagementPlugins: |
  - name: argocd-terraform-plugin-jsonnet
    generate:
      command: ["sh", "-c"]
      args: ["jsonnet . | argocd-terraform-plugin generate -"]
```
For sidecar configured plugins, add this to `cmp-plugin` ConfigMap, and then [add a sidecar to run it](../installation#initcontainer-and-configuration-via-sidecar):
```yaml
  avp-jsonnet.yaml: |
    ...
    spec:
      ...
      generate:
        command:
          - sh
          - "-c"
          - "jsonnet . | argocd-terraform-plugin generate -"
      lockRepo: false
```
...
<img src="https://github.com/KazanExpress/argocd-terraform-plugin/raw/main/assets/hard-refresh.png" width="300">

You can also use the `argocd app diff` command passing the `--hard-refresh` flag. This will run argocd-terraform-plugin again and pull in the new values from you Secret Manager and then you can either have Auto Sync setup or Sync manually to apply the new values.
...
#### Running argocd-terraform-plugin in a sidecar container
...
- Use an image that contains the binaries needed: if attempting to deploy Helm charts with argocd-terraform-plugin, ensure the image either contains Helm and argocd-terraform-plugin pre-installed, or has some logic to fetch it from somewhere. Unlike the `argocd-cm` ConfigMap based installation, the sidecar image is supplied by the user and is distinct from the one for the `argocd-repo-server`, which means previously pre-included binaries will be absent
